	...
	public void startNumer0n() {
		...
		while (true) {
			...
			if (!Const.NO_OPTION.equals(item)) {
				...
				if (optionResult == Const.Numer0n_GAMEOVER) {
					break;
				} else {
			...
			if (!Const.DOUBLE.equals(item)) {
				...
				if (Const.CPU.equals(this.gameMaster.getName())) {
					...
					this.info.addCpuInfoList(Anything.concatStringToComma(
							Const.NO_OPTION, Anything.concatListToComma(callNumber),
				...
				if (Eatbite.ALL_EAT.equals(ebResult)) {
					break;
				}
			}
			...
			if (Const.NO_OPTION.equals(item)) {
				...
				item = Const.NO_OPTION;
			}

			// 防御オプション使用時、optionクラスにアクセス
			if (Const.SHUFFLE.equals(item) ||
					Const.CHANGE.equals(item)) {
